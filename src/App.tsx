...
export default function App() {
      ...
      CapApp.addListener('appUrlOpen', (data) => {
        if (data.url.includes('access_token')) {
          ...
          if (token) {
            ...
            Browser.close(); // Fecha o navegador de login
          }
        }
      });
    }
  ...
  // Capturar retorno do Dropbox OAuth no navegador Web
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      <StatusBar isOnline={isOnline} isEncrypted={true} onToggleOnline={handleToggleOnline} onSync={handleSyncData} />
      <main className="max-w-6xl w-full mx-auto p-6 md:p-12 flex-1 flex flex-col gap-10">
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 text-sm font-medium ml-14">Arquitetura Limpa • <span className="text-slate-400">Privacidade Absoluta</span></p>
          </div>
