...
export const useFinanceApp = () => {
  ...
  const handleExportCSV = useCallback(() => { exportTransactionsToCSV(db.transacoes); showToast('CSV Exportado!', 'success'); }, [db.transacoes, showToast]);
  ...
  const handleGenerateInsights = async (force = false) => {
    ...
    addLog('DOMAIN', 'Analisando finanças com IA...');
    ...
    try {
      ...
      addLog('DOMAIN', 'Insights gerados.');
    } catch (err) { addLog('INFRASTRUCTURE', 'Falha nos insights.'); } finally { setIsAIAnalyzing(false); }
  };
  ...
  const handleProcessAICommand = async (text: string) => {
    ...
    const systemPrompt = `Extraia dados em JSON: {descricao, amount, category, paymentMethod, installments, date}. Categorias:\n${categoriesPrompt}`;
